...
class Common_model extends CI_Model {
	...
	function pagination($query,$per_page=100,$page=1,$url='?',$w=''){   
...
//=============List all records function starts=============//
	...
	public function fetchall($table, $arr=NULL, $range=NULL, $where=NULL, $groupby=NULL,$orderby=NULL,$like=NULL)
...
//============== Function to fetch a row starts===========//
		...
		public function get_info($tablename, $id=NULL, $field=NULL,$range=NULL,$whereArray=NULL){
		if($range){
			$this->db->select($range);
		}else{
			$this->db->select('*');
...
//============== Function to add/edit Reason starts===========//
	...
	public function reason_add(){
		...
		$data = array(
			...
			'reason_name' => $this->input->get_post('reasons')
		...
		if($this->input->get_post('id')!=''){
			...
			$query = $this->db->update('mm_reason_inaccessibly_tbl', $data);

				$this->session->set_userdata(array('msg' => 'Reason Updated.'));

		}else{

			$query = $this->db->insert('mm_reason_inaccessibly_tbl', $data);

				$this->session->set_userdata(array('msg' => 'New Reason Added.'));

		}

	}

//============== Function to add/edit Reason ends===========//
